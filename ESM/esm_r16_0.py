...
Based on ESM_r16 (renamed at the start of a new round on branch esm-r0-cellru-mapping - adds
...
class ESAnalyzerApp(AppDashboard):
    ...
    def _calc_es_level_simulation_savings(self, summary_df, start_date, end_date, eval_hours=None):
        ...
        결합해 Sector별 예상 절감 에너지[Wh]를 계산하고, Energy Stat 데이터에서 동일 평가 기간·평가
        시간(eval_hours)의 실제 총 소모 에너지[Wh]를 가져와 절감률/Tput 위반 비율과 함께 정리한다.
        마지막 행에 전체 사이트 합계를 추가한다.
        ...
        적용하고, "Applied_ES_Level > k"(이미 현재 레벨보다 낮아 최소 한 스텝 이상 여유가 있는 구간)인
        구간에는 RU HW가 Deep Sleep을 지원하는 경우 Deep Sleep 기준(더 큰) 전력차를 적용한다(지원하지
        않으면 이 구간도 그냥 PA off 기준과 동일). 즉:
