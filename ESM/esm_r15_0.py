...
[r0-후속] 단순화: eMTC/ENDC anchor 속성 조회를 cm_map_df_full 단일 조회로 단순화:
  - **배경**: 위 [r0]에서 `conf-emtc-switch`/`endc-anchor-type`/`endc-support`의 출처가 CarrierConf인지
    CM Data인지 불확실해 방어적으로 이중 조회하도록 구현했었는데, 사용자가 실제 데이터 구조를 확인해줌:
    "conf-emtc-switch/endc-anchor-type/endc-support는 병합된 CM데이터에 있어. eNodeBID, cell-num, Sector
    열(인덱스)을 가지고 있어서, SectorList와 CarrierConf를 이용하면, 각 cell-num-Band를 연결할 수 있고,
    band별 priority 값들도 연결할 수 있어." 즉 이 세 속성은 CarrierConf가 아니라 CM Data(cm_map_df_full,
    eNodeBID+cell-num 인덱스)에만 있고, CarrierConf는 band<->cell-num 매핑(cell-num-A/B/C/D)과 우선순위
    값(ES priority/ENDC_priority)만 제공한다는 것이 확정됨.
  - **수정**: `_lookup_cell_attr(carrier_row, enodeb, cell_num, col_name)` -> `_lookup_cell_attr(enodeb,
    cell_num, col_name)`로 단순화(CarrierConf 조회 분기 제거, cm_map_df_full에서 eNodeBID+cell-num으로만
    조회). `_is_emtc_enabled`/`_is_endc_anchor_enabled`도 `carrier_row` 인자를 제거하고 2-인자로 변경.
    `_enforce_protected_band` 내부에서 `is_enabled_fn`을 호출하는 부분만 인자 수를 맞춰 수정 - band<->
    cell-num 매핑과 priority 조회(carrier_df 기반)는 그대로 유지.
  - **검증**: 단위 테스트를 새 데이터 모델(conf-emtc-switch/endc-anchor-type/endc-support는
    cm_map_df_full에, ES priority/ENDC_priority는 carrier_df에)에 맞춰 다시 작성해 재실행 - 서비스
    미배치/이미 보장됨/강제 조정(eMTC·ENDC anchor)/anchor-type 불일치·support=False 케이스 모두 통과.
    `run_analysis` 합성 데이터 통합 테스트도 conf-emtc-switch를 cm_map_df_full에 두도록 수정해 재실행,
    동일하게 통과. `python -m py_compile` 통과.

[r0-후속2] 기능 개선: ES Level이 아예 생성되지 않은 sector도 에너지 절감효과 결과표에 포함(절감 0):
  - **배경**: eMTC/ENDC anchor 강제 조정 등으로 어떤 sector가 ES 적용대상 cell/band를 하나도 찾지 못해
    ES Level 1조차 생성되지 못하는 경우가 생길 수 있음. 사용자 요청: "ES 적용대상 cell/band를 찾지
    못해 ES level 1도 생성되지 않은 경우, 시뮬레이션을 수행할 필요는 없어. 다만 시뮬레이션 기반으로
    Energy Saving 이득을 계산하니까, 에너지 절감효과 등을 계산할 때는 ES level이 없더라도 SectorList에
    있는 sector들을 모두 포함하도록 코드를 수정해줘." — `_run_es_level_simulation`이 Max_ES_Level<=0인
    sector를 애초에 건너뛰므로(시뮬레이션 자체는 이미 정상적으로 불필요), `_calc_es_level_simulation_savings`가
    그 결과(summary_df)만 순회하면서 그런 sector가 통째로 결과표에서 빠지는 문제가 있었음.
  - **수정**: 새 헬퍼 `_get_all_sectorlist_pairs()`가 현재 Target Cat_ID의 SectorList에 있는 모든
    (eNodeBID, Sector) 쌍을 반환. `_calc_es_level_simulation_savings`는 summary_df에 있는 sector는
    기존과 동일하게 절감 에너지를 계산하고, SectorList에는 있지만 summary_df에 없는 sector는 소모
    에너지(Energy Stat 기반, ES 정책과 무관하게 계산 가능)만 채우고 절감에너지=0인 행을 추가한다.
    summary_df가 완전히 비어 있어도(=모든 sector가 ES Level 없음) 빈 표를 반환하던 것을 SectorList
    전체를 0-절감 행으로 채운 표를 반환하도록 변경(기존에는 `summary_df is None or empty`일 때 즉시
    빈 DataFrame을 반환했음). 전체 합계 행은 기존 로직 그대로 재사용(모든 행의 합이라 자동으로 반영됨).
  - **적용 범위**: `_calc_es_level_simulation_savings`(시뮬레이션 기반, Advanced Settings 기본값인 Mode
    2/3에서 사용)에만 적용 — NC2 기반(`_calc_all_savings`, Mode 1)은 ES Level별 상세 행(Target Cell
    Num/Board Type 등) 구조라 의미 있는 0-행을 만들기 어렵고, 사용자 요청도 "시뮬레이션 기반" 계산을
    명시적으로 지목해 이번에는 다루지 않음.
  - **검증**: SectorList에 3개 sector(1개는 ES 정책 있음, 2개는 ES-적용대상 band 없음)를 두고
    `_calc_es_level_simulation_savings`를 호출 - 정책 없는 2개 sector도 결과표에 절감에너지 0, 소모
    에너지는 정상값으로 나타나고 전체 합계도 3개 sector 소모량을 모두 반영함을 확인. summary_df를
    완전히 비워도(모든 sector가 ES Level 없음) SectorList 3개 sector가 모두 0-절감 행으로 채워진 표를
    반환함을 확인(빈 DataFrame이 아님). `python -m py_compile` 통과.

[r0-후속3] 버그 수정 + 기능 개선: ES 윈도우 진입 시 최소 1스텝 ES 미적용 강제 문제 수정 + RBlow를
CoveragePriority 기반 Coverage band 선택으로 변경:
  - **배경 1(윈도우 진입 버그)**: 사용자가 "상당히 많은 sector에서 ES가 미적용된 시간이 1 time
    sample로 잡히는 경우가 매우 많은데 첫번째 time에 대한 데이터가 없어 1회 빠지는 거야?"라고 확인
    요청. 확인 결과: `_run_es_level_simulation`의 IIR(cm_iir/rb_iir)은 윈도우 경계와 무관하게 전체
    기간 연속으로 갱신되고 있었는데도, 윈도우 진입 첫 스텝(`w_prev != w_now`)에서는 이미 충분한
    IIR값(직전 15분치)이 있는지 확인하지도 않고 무조건 Level 0으로 고정하고 있었음 - 이 강제 리셋이
    매 윈도우(=거의 매일)마다 반복되므로 "많은 sector에서 ES 미적용 1 sample" 패턴이 나타난 것.
  - **수정 1**: 사용자 지시대로 "시작시점 기준 이전 데이터가 있다면 전체 데이터에 대해 IIR을 하고
    그 직전 15분 값을 기준으로 시작시점의 진입을 결정, 없다면 시작일 00:00 값을 기준으로 판단"하도록
    변경 - 윈도우 진입 첫 스텝을 curr=0으로 취급하되(그 전 레벨을 그대로 이어받지는 않음) 더 이상
    무조건 Level 0으로 고정하지 않고, 곧바로 같은 Entering 조건 평가(직전 스텝의 연속 IIR 사용)를
    적용해 조건이 이미 충족되면 그 스텝부터 바로 Level 1로 진입한다. 시뮬레이션 구간의 맨 첫 행(i=0,
    참조할 이전 행이 아예 없음)은 그 행 자체의 raw 값(=00:00 값)을 기준으로 동일하게 판단한다.
  - **배경 2(RBlow 고정값 문제)**: "RBlow도 이제 고정값이 아니라 설정을 기준으로 선택해보자. endc
    anchor처럼 CarrierConf의 CoveragePriority를 기준으로 우선순위에 따라 선택해야해. Coverage band가
    되며, 이 주파수/band에는 ES를 적용하면 안되는거야. 그리고 선택된 band의 nRB*RBlow multiplier_lte
    (0.5)로 RBlow값을 계산해야해." 기존 코드는 present band 중 CoveragePriority가 가장 좋은 band를
    골라 그 RB값으로 RBlow를 계산하긴 했지만(`cov_sorted`/`cov_band_rb`), 선택된 band를 ES 적용대상
    에서 실제로 제외하지는 않아 그 band에도 ES가 그대로 적용될 수 있었음(Coverage 기준 band가 스스로
    사라지는 모순).
  - **수정 2**: 새 헬퍼 `_select_coverage_band()`가 eMTC/ENDC anchor 강제 조정 다음 순서로 실행 -
    present band 중 CoveragePriority(동률 시 ES capability -> ES priority)가 가장 좋은 band를
    Coverage band로 선택하고, 이미 RB<0이면 그대로, RB>0이면 강제로 RB<0 전환(ES 적용대상에서 제외).
    `rb_low = np.ceil(coverage_rb * rb_mult)`로 계산(공식 자체는 기존과 동일, `rb_mult` =
    `RBlow Multiplier_LTE`). 기존 `present_bands_names`/`cov_candidates`/`cov_sorted`/`cov_band_rb`
    블록은 제거.
  - **검증**: 윈도우 진입 관련 4가지 케이스(조건 충족 시 윈도우 진입 즉시 Level 1 진입/조건 미충족 시
    기존과 동일하게 Level 0 유지/데이터셋 맨 첫 행이 윈도우 안일 때 동일하게 즉시 진입/조건 미충족 시
    유지)를 모두 확인. Coverage band 선택 단위 테스트로 (a) 이미 RB<0인 최우선 band는 무변경, (b)
    RB>0인 최우선 band는 강제 전환, (c) CoveragePriority 동률 시 ES capability로 타이브레이크, (d)
    present band 없음 시 무동작을 확인. `run_analysis` 합성 데이터로 Coverage band가 실제로 ES
    적용대상에서 제외되고(ES Level 7에만 등장) RBlow가 `선택된 band nRB * multiplier`와 일치함을
    확인. 기존 eMTC/ENDC anchor 강제 조정, 전 SectorList 포함(절감 0) 테스트도 재실행해 회귀 없음을
    확인. `python -m py_compile` 통과.
"""
