...
## 5-1. 새 라운드: esm_r17.py (esm_r15_0.py/esm_r16_0.py는 브랜치 esm-r0-cellru-mapping에 이 시점 상태로 보존)

* **[라운드 전환] (2026-07-09)** 사용자 지시로 `esm_r16_0.py`(이미 esm_r15_0.py의 모든 수정 + Deep Sleep을 포함하고 있어 "r15_0/r16_0 둘 다 고려"하는 시작점으로 채택)를 복제해 `esm_r17.py`를 생성. 이 시점부터 `esm_r17.py`가 유일한 활성 개발 파일이며, `esm_r15_0.py`/`esm_r16_0.py`는 더 이상 수정하지 않고 이 라운드 종료 시점 상태로 보존한다.

* **[v5.11 / esm_r17.py 전용] (2026-07-09) Deep Sleep 기능 전면 재작성**
  * **배경**: 사용자가 "r16_0에 추가된 Deep Sleep 기능에 오류가 있어 보인다"며 상세 스펙 제공. 기존 구현(`_power_deltas_for_level`)의 문제: (1) Deep Sleep이 항상 켜져 있어 옵션으로 끌 수 없었음. (2) "RU 공유" 판정이 그 레벨의 Target Cell Num(=한 band의 셀)에만 국한되어, 정작 그 RU를 공유하는 *다른* band/셀(같은 sector든 다른 sector든)이 지금 이 순간에도 다 꺼져 있는지는 전혀 확인하지 않아 과다 절감 위험이 있었음.
  * **수정 1(옵션화)**: `self.deep_sleep_enabled`(Optimizer Advanced Settings 신규 체크박스, 기본값 off) 추가. On일 때만 `run_analysis`가 ESM Output Result에 `Deep Sleep Capability` 열을 추가(off면 열 자체를 만들지 않아 하위 시스템이 열 존재 여부로 식별).
  * **수정 2(Deep Sleep Capability 판정, 신규 `_compute_deep_sleep_capability`)**: RU 공유는 병합 CM 데이터의 (ru-board-type, ru-port-id) 조합으로 판정(사용자 확인: ru-board-id가 없는 셀도 있어 이 값이 없으면 어차피 board-type도 몰라 Deep Sleep 판정이 불가능하므로, 더 정밀한 'RU Path 공유'(board-id+port-id+cascade-id)는 이 그룹의 부분집합일 수밖에 없어 board-type+port-id만으로 충분). 한 그룹이 Deep Sleep 가능하려면: (a) board-type이 RU/MMU Spec에 'Deep Sleep' 소모전력 > 0으로 등록, (b) 그룹의 모든 cell이(다른 sector라도) 실제 ES Level을 배정받음(ES Level 7/no-policy이거나 결과에 없는 cell이 하나라도 있으면 그룹 전체 불가), (c) 모든 cell의 연결된 band의 CarrierConf `ES priority`가 0이 아님. eNodeBID 내 unique한 숫자 그룹 id(1부터, 불가능하면 -1) 부여.
  * **수정 3(전이 조건·절감량 재계산)**: "현재 최고 레벨(Curr)은 PA off, 그보다 낮은 레벨은 Deep Sleep" 원칙(5분 기상 지연 우려)은 유지하되, "그보다 낮은 레벨"이라고 곧바로 Deep Sleep을 적용하지 않고 그 레벨의 Deep Sleep Capability 그룹의 다른 멤버들이 "바로 이 15분 타임스탬프에" 전부 꺼져 있는지 확인한다 - `_run_es_level_simulation`의 `timeline_df`(그동안 버려지던 반환값)를 이제 저장해 eNodeBID 내 Sector들을 Time 기준으로 피벗해 매 15분 스텝마다 교차 확인(다른 Sector 데이터가 없으면 보수적으로 항상 미충족). 조건 미충족 스텝은 PA off로 폴백. Deep Sleep이 꺼져 있으면 이 판정을 생략하고 기존 count(Level>=k) 공식과 동일(하위 호환).
  * **검증**: `_compute_deep_sleep_capability` 단위 테스트(교차 Sector 그룹 id 일치, ES priority==0 배제, ES Level 없는 cell 배제, HW 미지원 배제) 통과. 손계산한 2-Sector 교차 시나리오로 `_calc_es_level_simulation_savings`를 직접 호출 - Sector 0(140Wh, Deep Sleep 보너스 40Wh)과 Sector 1(70Wh, 보너스 0)이 손계산과 정확히 일치, Sector 1이 그룹 내에서 잠깐 다시 켜지는 fallback 케이스로 보너스가 정확히 그만큼 줄어듦(40→30Wh)을 확인해 진짜 15분 단위 교차 동기화가 동작함을 증명. 기존 eMTC/ENDC anchor, 전 SectorList 포함, 윈도우 진입, Coverage band, 설정 자동 저장/불러오기, Output 폴더 통합 회귀 테스트 전체 재실행해 모두 통과(Deep Sleep 옵션이 꺼진 상태로 실행되므로 하위 호환 경로 자동 검증). `python -m py_compile` 통과.
  * **적용 범위**: `esm_r17.py` 전용(새 라운드의 유일한 활성 파일).

## 5. 진행 중인 작업 및 다음 단계 (To-Do / Next Steps)

* 현재 상태: v5.11(`esm_r17.py` 전용, 새 라운드) - Deep Sleep 기능을 옵션화(기본 off)하고, RU 공유 판정을 (ru-board-type, ru-port-id) 기준으로 재정의했으며, Deep Sleep 전이 조건을 15분 타임스탬프 단위 교차-Sector 동기화로 재계산하도록 전면 재작성. 이전 v5.10(`esm_r15_0.py`+`esm_r16_0.py`)에서 ES 윈도우 진입 시 무조건 1스텝 ES 미적용으로 대기하던 버그를 수정하고 RBlow를 CoveragePriority 기반 Coverage band 선택으로 변경했고, v5.9에서 ES Level이 아예 생성되지 않은 sector도 에너지 절감효과 결과표에 절감 0으로 포함되도록 수정했고, v5.8에서 eMTC/ENDC anchor 속성 조회처가 CM Data로 확정되어 CarrierConf 폴백 조회를 제거·단순화했고, v5.7에서 Cell-RU 매핑 저장/자동 불러오기 기능과 eMTC/ENDC anchor 서비스 보장을 위한 ES 적용대상 band 강제 조정 규칙을 추가했고, v5.6(`esm_r15.py`+`esm_r16.py`, 구 라운드)에서 설정 자동 저장/불러오기 + Input 폴더 파일 자동 선택 + Output 폴더 통합 편의 기능을 반영하며 구 라운드를 마무리했고, v5.0(`esm_r16.py`)에서 Deep Sleep 기능을 신규 추가하며 여기까지 도달.
* 확인 필요:
  ...
  15. **다음 결정 대기(v5.11, 중요)**: Deep Sleep 기능을 실제로 켜서(Advanced Settings에서 On) 실데이터로 검증 필요 — 특히 (a) CM 데이터의 board-type 컬럼명이 코드가 찾는 패턴('boardtype'/'ruboardtype' 정규화 매칭)과 일치하는지, (b) RU를 공유하는 cell들이 실제로 여러 sector에 걸쳐 있는 사례가 있는지와 그 경우 Deep Sleep Capability 그룹 id가 기대한 대로 묶이는지, (c) `_run_es_level_simulation`이 호출되는 3개 지점(에너지 분석 실행/절감효과 예측/ES Level 시뮬레이션 팝업) 중 raw_df가 특정 eNodeBID·Sector로 필터링된 상태로 호출되는 경우(예: 에너지 분석 실행에서 특정 sector만 선택) 교차-Sector 판정에 필요한 다른 sector의 timeline이 raw_df에 아예 없어서 "보수적으로 항상 미충족" 처리되어 실제보다 절감량이 낮게 나올 수 있음 — 이 케이스가 실사용에 문제가 되는지 확인 필요(문제가 되면 해당 호출부에서 raw_df 필터링 이전에 eNodeBID 전체를 시뮬레이션하도록 조정 필요).
